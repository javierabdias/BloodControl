...
public class Cita_Tabla extends Task <ObservableList<Cita>> {
    ...
    protected ObservableList<Cita> call() throws Exception {
        ...
        Query query = em.createQuery("SELECT NEW com.integrador.POJOLista.Cita (B.perNombre,B.perAp,B.perAm,C.citFecha,C.citHora,E.status,D.status) FROM Paciente A, Persona B, Citas C, StatusExa D, StatusPag E "
                + "WHERE A.perId=B.perId AND A.pacId=C.pacId AND E.stapId=C.stapId AND C.staeId=D.staeId AND A.erId='A'");
